...
async def detect(event):
    ...
    event = await edit_or_reply(event, "**MenDownload file untuk diperiksa...**")
    media = await event.client.download_media(reply)
    if not media.endswith(("png", "jpg", "webp")):
        return await edit_delete(event, "**Mohon Reply ke gambar atau stiker!**", 90)
    event = await edit_or_reply(event, "**Detecting NSFW limit...**")
    r = requests.post(
...
CMD_HELP.update(
    {
        ...
        \nPenggunaan: Untuk mendeteksi konten 18+ dengan gambar balasan.\
    "
    }
)
